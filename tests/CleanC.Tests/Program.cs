...
sealed class Tests
{
 ...
 public async Task Run()
 {
  ...
  await Test("Recent browser cache is optional",()=>{var baseDir=Path.Combine(root,"browser-recent","Cache");var rule=new CleanupRule("Chrome-Test-Cache",baseDir,"Chrome 浏览器缓存",TimeSpan.FromDays(7));var p=new SafetyPolicy([rule]);Check(p.Classify(SnapshotAge(Path.Combine(baseDir,"entry.bin"),1),DateTime.UtcNow).Safety==SafetyLevel.Optional);});
  await Test("Browser cache older than seven days is safe",()=>{var baseDir=Path.Combine(root,"browser-old","Cache");var rule=new CleanupRule("Chrome-Test-Cache",baseDir,"Chrome 浏览器缓存",TimeSpan.FromDays(7));var p=new SafetyPolicy([rule]);Check(p.Classify(SnapshotAge(Path.Combine(baseDir,"entry.bin"),10),DateTime.UtcNow).Safety==SafetyLevel.Safe);});
  ...
  await Test("Missing selected file is resolved instead of skipped",async()=>{var p=Old("temp/gone-before-clean.tmp");var item=Item(p);File.Delete(p);var r=await Cleaner().ExecuteAsync([item],false,null,CancellationToken.None);Check(r.Skipped==0&&r.Deleted==1&&r.Items.Single().Result=="Gone");});
